...
where:
- \(y_1\) is the distance from the wall to the center of the first cell
...
\[
u_\tau \approx U_b \sqrt{\frac{C_f}{2}}
\]

where \(U_b\) is a representative bulk velocity and \(C_f\) is a skin-friction coefficient.

For smooth turbulent channel/duct flows, a common engineering approximation is:

\[
C_f \approx 0.079 \, Re^{-0.25}, \quad Re = \frac{U_b H}{\nu}
\]
...
Example values:
- \(\nu = 1\times10^{-6}\,\mathrm{m^2/s}\) (water-like)
- \(U_b = 1\,\mathrm{m/s}\)
- \(H = 0.02\,\mathrm{m}\)

\[
Re = \frac{1 \cdot 0.02}{10^{-6}} = 2\times10^{4}
\]
\[
C_f \approx 0.079(2\times10^4)^{-0.25} \approx 0.0066
\]
\[
u_\tau \approx 1\sqrt{\frac{0.0066}{2}} \approx 0.058\,\mathrm{m/s}
\]

For a target \(y^+=50\):

\[
y_1 \approx \frac{50 \cdot 10^{-6}}{0.058} \approx 8.7\times10^{-4}\,\mathrm{m}
